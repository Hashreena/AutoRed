...
def build_nmap_command(target, profile, preset='quick'):
    ...
    # CVSS threshold per profile — keeps findings relevant
    mincvss = {
        'Production': '9.0',   # Critical only
        'Standard':   '7.0',   # High + Critical
        'Deep':       '4.0',   # Medium and above
    }.get(profile, '7.0')
...
def build_theharvester_command(target, profile, preset='quick'):
    ...
    return (
        f"theHarvester -d {target} -b {sources} "
...
if __name__ == '__main__':
    ...
    for target in targets:
        print(f"\n===== TARGET: {target} =====")
        for profile in ['Production', 'Standard', 'Deep']:
            for tool in tools:
                cmd = build_command(
                    tool, target, profile, 'quick'
                )
                print(f"[{profile}/{tool}] {cmd}")
