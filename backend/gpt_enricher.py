...
{{
  ...
  "exposure_type": "Protocol Weakness|Configuration Weakness|Vulnerability|Information Disclosure|Authentication Weakness",
...
if __name__ == '__main__':
    ...
    tests = [
        ...
        },
        {
            ...
            'description': 'PostgreSQL with empty password',
            'evidence':    '',
        },
        {
            ...
            'description': 'PHP easter egg ?=PHPB8B5F2A0-D9D8-2CF2-3B5B-F5B3D5E5D5E5 found',
            'evidence':    '',
        },
        {
            ...
            'severity':    'Low',
            'asset':       '192.168.112.130:80',
            'description': 'Apache mod_negotiation allows filename guessing',
            'evidence':    '',
        },
    ]
    ...
    for t in tests:
        ...
        if result:
            ...
            print(f"  CWE Name:     {result.get('cwe_name')}")
            print(f"  Exposure:     {result.get('exposure_type')}")
            print(f"  MITRE:        {result.get('mitre_technique')}")
            print(f"  Exploitability: {result.get('exploitability')}")
            print(f"  Confidence:   {result.get('confidence')}")
            expl = result.get('ai_explanation', '')[:120]
