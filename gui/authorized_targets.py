...
class AuthorizedTargetsManager(QWidget):
    ...
    def apply_theme(self, prefs):
        ...
        for attr in (
            ...
            if hasattr(self, attr):
                pending[attr] = getattr(self, attr).text()
    ...
    def init_ui(self):
        ...
        self.target_input = self._make_input(
            "Target domain or IP, e.g. maybank.com or 192.168.1.1"
        )
        form_layout.addWidget(self.target_input)

        self.auth_by_input = self._make_input(
            "Authorized by, e.g. John Smith — CISO, CyberShield Sdn Bhd"
        )
        form_layout.addWidget(self.auth_by_input)

        self.auth_email_input = self._make_input(
            "Authorizer email — approval code will be sent here"
        )
        form_layout.addWidget(self.auth_email_input)

        self.engagement_input = self._make_input(
            "Engagement name, e.g. Q3 2025 Pentest — External Assessment"
        )
        form_layout.addWidget(self.engagement_input)

        self.notes_input = self._make_input(
            "Notes, e.g. Ref: SOW-2025-001, scope limited to web apps only"
        )
        ...
        self.outer.addWidget(container)
