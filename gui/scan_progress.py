...
class ScanProgressScreen(QWidget):
    ...
    def send_completion_notifications(self):
        try:
            # ── Read recipient from .env — never hardcode ──
            ...
            if os.path.exists(env_path):
                with open(env_path) as f:
                    for line in f:
                        line = line.strip()
                        if line.startswith('NOTIFY_TO='):
                            notify_to = line.split('=', 1)[1].strip()
                            break
            # Fall back to environment variable
            if not notify_to:
                notify_to = os.environ.get(
                    'NOTIFY_TO', 'configured recipient'
                )
            self.append_log("")
            self.append_log(
                f"[*] Sending scan report to {notify_to} ..."
            )
